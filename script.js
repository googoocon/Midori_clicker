...
function init() {
    ...
    let nihahahaSoundSrc = './sounds/Midori1.mp3';
    let uwahSoundSrc = './sounds/Midori2.mp3';
    let isBgmOn = false;
    bgm.volume = 0.8;
    ...
    function handleBodyClick(event) {
        ...
        const iconType = Math.random() < 0.9 ? 'mmmm' : 'mnmn';
        playSound(iconType);
        addIcon(event, iconType);
    }

    function playSound(type) {
        const sound = new Audio(type === 'mmmm' ? nihahahaSoundSrc : uwahSoundSrc);
        sound.play();
    }
